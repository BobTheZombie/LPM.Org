...
## One-command BLFS-style live system

The `systemiso` command builds the selected source recipes in dependency order,
installs each generated LPM package into a target root, configures networking,
generates the live initramfs with mkinitcpio, and creates the GRUB ISO:

```sh
sudo lpm systemiso \
  --lpmbuild-root ./packages \
  --package-profile ./packages/profiles/live-x86_64-v2.txt \
  --root ./build/lpm-root \
  --artifact-dir ./build/packages \
  --architecture x86_64-v2 \
  --output ./dist/lpm-live-x86_64-v2.iso
```

The package repository's mkinitcpio recipe installs the `lpm-live` hook. At
boot it locates the ISO by volume label, mounts `live/rootfs.squashfs`, creates
a writable tmpfs OverlayFS layer, and switches into the resulting live root.
The GitHub Actions workflow `Build LPM live ISO` runs the same command and
publishes the ISO, SHA-256 checksum, and JSON build manifest as artifacts.
