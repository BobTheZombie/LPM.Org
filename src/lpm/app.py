Warning: truncated output (original token count: 68857)
Total output lines: 7314
...
# =========================== Snapshots =====================================
_SNAPSHOT_META = ".lpm-snapshot.json"


class _NonClosingWriter:
    """Let zstd finalize without closing atomic_replace's file handle."""
...
def _installed_row(conn: sqlite3.Connection, name: str) -> Optional[dict]:
    row = conn.execute(
        "SELECT name,version,release,arch,provides,symbols,requires,manifest,explicit,install_time "
        "FROM installed WHERE name=?", (name,),
    ).fetchone()
    if row is None:
        return None
    keys = ("name", "version", "release", "arch", "provides", "symbols", "requires",
            "manifest", "explicit", "install_time")
...
def create_snapshot(
    ...
    *,
    ...
    """Save pre-transaction files and package database rows.

    Archives live in VERSION_STORE_DIR, which can be a separate mount via
    LPM_VERSION_STORE_DIR.  Metadata records paths which did *not* exist, so a
    rollback can remove files introduced by the failed/new package version.
    """
    ts = int(time.time())
    safe_tag = re.sub(r"[^A-Za-z0-9._-]", "_", tag)
    root = Path(root).resolve()
    archive_dir = VERSION_STORE_DIR / "transactions"
    archive = archive_dir / f"{ts}-{time.time_ns() % 1_000_000_000:09d}-{safe_tag}.tar.zst"
    ...
    for value in files:
        ...
        try:
            ...
            raise ValueError(f"snapshot path escapes target root: {path}")
        if not rel or rel == "." or rel in seen:
            continue
        seen.add(rel)
        normalized.append((path, rel))
    conn = db()
    package_state = {name: _installed_row(conn, name) for name in sorted(set(packages))}
    conn.close()
    metadata = {
        "format": 2,
        "root": str(root),
        "tag": tag,
        "paths": [
            {"path": rel, "existed": path.exists() or path.is_symlink()}
            for path, rel in normalized
        ],
    ...
    with operation_phase(privileged=True):
        ...
        _apply_state_permissions(VERSION_STORE_DIR, directory=True)
        _apply_state_permissions(archive_dir, directory=True)
        with atomic_replace(archive, mode=0o644, open_mode="wb") as fh:
            # atomic_replace must retain ownership of the underlying file so it
            # can flush, fsync, and rename it after zstd finalizes the frame.
            with cctx.stream_writer(_NonClosingWriter(fh)) as compressor:
                with tarfile.open(fileobj=compressor, mode="w|") as tf:
                    payload = json.dumps(metadata, sort_keys=True).encode("utf-8")
                    info = tarfile.TarInfo(_SNAPSHOT_META)
                    info.size = len(payload)
                    info.mode = 0o600
                    info.mtime = ts
                    tf.addfile(info, io.BytesIO(payload))
                    for p, rel in normalized:
...
def restore_snapshot(archive: Path, *, root: Optional[Path] = None) -> dict:
    archive = Path(archive)
    metadata: Optional[dict] = None
    target_root = Path(root or DEFAULT_ROOT).resolve()

    def remove_new_paths(snapshot_meta: dict) -> None:
        for entry in sorted(
            snapshot_meta.get("paths", []),
    ...
    def restore_member(tf: tarfile.TarFile, member: tarfile.TarInfo) -> None:
        pure = Path(member.name)
        if pure.is_absolute() or ".." in pure.parts:
            raise ValueError(f"unsafe snapshot member: {member.name}")
        dest = target_root / pure
        ...
        if member.isdir():
            ...
            if source is None:
                raise ValueError(f"snapshot member has no payload: {member.name}")
            with source, dest.open("wb") as out:
                shutil.copyfileobj(source, out, length=IO_BUFFER_SIZE)
        if not member.issym():
            with contextlib.suppress(OSError):
                os.chmod(dest, member.mode)
        with contextlib.suppress(OSError):
            os.chown(dest, member.uid, member.gid, follow_symlinks=False)
        ...
        with archive.open("rb") as f:
            with dctx.stream_reader(f) as reader:
                with tarfile.open(fileobj=reader, mode="r|") as tf:
                    for member in tf:
                        if member.name == _SNAPSHOT_META:
                            source = tf.extractfile(member)
                            metadata = json.loads(source.read().decode("utf-8")) if source else None
                            if metadata:
                                target_root = Path(root or metadata.get("root", DEFAULT_ROOT)).resolve()
                                remove_new_paths(metadata)
                            continue
                        restore_member(tf, member)
    if metadata:
        conn = db()
        with conn:
            for name, row in metadata.get("packages", {}).items():
                if row is None:
                    conn.execute("DELETE FROM installed WHERE name=?", (name,))
                else:
                    conn.execute(
                        "REPLACE INTO installed(name,version,release,arch,provides,symbols,requires,manifest,explicit,install_time) "
                        "VALUES(:name,:version,:release,:arch,:provides,:symbols,:requires,:manifest,:explicit,:install_time)", row,
                    )
        conn.close()
    return metadata or {"format": 1, "root": str(target_root)}
...
# =============== BUI…18857 tokens truncated…h.name.lower()
...
def cmd_install(a):
    ...
    with _delta_mode(mode):
        ...
        try:
            ...
            snapshot_archive = create_snapshot(
                tag, affected, root=root, packages=[p.name for p in plan]
            )
            conn = db()
...
def cmd_remove(a):
    ...
    if not a.dry_run:
        ...
        snapshot_archive = create_snapshot(tag, affected, root=root, packages=a.names)
        conn = db()
        row = conn.execute("SELECT id FROM snapshots WHERE archive=?", (snapshot_archive,)).fetchone()
        conn.close()
        if row:
...
def cmd_upgrade(a):
    ...
    with _delta_mode(mode):
        ...
        if not dry:
            ...
            snapshot_archive = create_snapshot(
                tag, affected, root=root, packages=[p.name for p in upgrades]
            )
            conn = db()
            row = conn.execute("SELECT id FROM snapshots WHERE archive=?", (snapshot_archive,)).fetchone()
            conn.close()
...
def cmd_rollback(a):
    ...
    sid, tag, archive = row
    restore_snapshot(Path(archive), root=Path(a.root) if a.root else None)
    conn.execute(
...
def build_parser()->argparse.ArgumentParser:
    ...
    sp=sub.add_parser("rollback", help="Restore package files and database state from a version snapshot"); sp.add_argument("snapshot_id", nargs="?", type=int, help="snapshot ID (default latest)"); sp.add_argument("--root", help="target root (defaults to the root recorded in the snapshot)"); sp.set_defaults(func=cmd_rollback)
