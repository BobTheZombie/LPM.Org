...
def _ensure_default_base(base: Iterable[str]) -> List[str]:
    ...
    impossible to opt-out of the built-in ``lpm-base`` and ``lpm-core`` goals
    which, in turn, caused confusing resolution errors for downstream projects
    that provide their own base package set.  By short-circuiting when *base*
    already contains entries we allow those projects to fully override the
    defaults while still falling back to ``DEFAULT_BOOTSTRAP_BASE`` when
